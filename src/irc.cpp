...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #neocoincoinTEST\r");
            Send(hSocket, "WHO #neocoincoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #neocoincoin\n");
            // randomly join #neocoincoin00-#neocoincoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #neocoincoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #neocoincoin%02d\r", channel_number).c_str());
        };
